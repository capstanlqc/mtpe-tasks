...
import Levenshtein  # normalized?? edit distance # as percentage
...
def split_note_column(df):
    ...
    df[["score", "category"]] = df["note"].str.extract(
        r"^(\d+\.\d+):\s*((?:LOW|HIGH) CONFIDENCE$)"
    )
...
def add_postedited_version(mt_tmx_fpath, pe_tmx_fpath):
    ...
    return pd.merge(
        mt_df, pe_df[["source_sentence", "pe_version"]], on="source_sentence"
    )


def add_similarity_ratio(df):
    ...
    df["edit_distance"] = df.apply(
        lambda row: Levenshtein.distance(row["mt_version"], row["pe_version"]), axis=1
    )
    df["similarity_ratio"] = df.apply(
        lambda row: Levenshtein.ratio(row["mt_version"], row["pe_version"]), axis=1
    )
...
def add_edit_distance(df):
    ...
    df["edit_distance"] = df.apply(
        lambda row: Levenshtein.distance(row["mt_version"], row["pe_version"])
        / max(len(row["mt_version"]), len(row["pe_version"])),
        axis=1,
    )
    df["similarity_ratio"] = df.apply(
        lambda row: Levenshtein.ratio(row["mt_version"], row["pe_version"]), axis=1
...
def draw_plots(data, target_lang):
    ...
    plt.title(f"Edit effort vs. QE score {target_lang}")
    ...
    # plt.show()
    plt.savefig(
        f"analysis/ned_vs_score_dev_{target_lang}.png", dpi=300, bbox_inches="tight"
    )
...
def save_data_as_excel_one_lang_per_sheet(multilingual_data, filename):
    output_file = f"analysis/{filename}.xlsx"
...
def save_results_as_excel(data, filename):
    print(results)
    output_file = f"analysis/{filename}.xlsx"
...
for omtprj_dpath in omtprj_dpaths:
    ...
    if len(mt_tmx_fpaths) == 0:
        print(
            f"No MT memory found in {os.path.basename(omtprj_dpath)}, unable to draw initial MT used!"
        )
        continue
    ...
    pearson_corr, pearson_pval = pearsonr(data["similarity_ratio"], data["score"])
    spearman_corr, spearman_pval = spearmanr(data["similarity_ratio"], data["score"])
...
save_data_as_excel_one_lang_per_sheet(data_all_lang, "data_ed_normalized")
save_results_as_excel(results, "results_ed_normalized")
